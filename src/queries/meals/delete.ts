...
function useMealDelete(
    ...
    const{mutate, error, isLoading} = useMutation(deletionKey, async({id, token}: {id:string, token: string}) => {
            ...
            await axios.delete(apiRoutes.DELETE_MEAL(id), {headers: {Authorization: `Bearer ${token}`}})
        }, {onSuccess: () => queryClient.invalidateQueries({queryKey: ['MealsList']}) , onError: (err: AxiosError) => console.error('Meal not deleted!', err)}
    )
